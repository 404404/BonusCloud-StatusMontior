...
# 版本信息，每次更新版本必须更改
printf "Version: v1.5-arm \t\t"
# 版本信息，每次更新版本必须更改
kel_v=$(uname -r | grep -q 'aml' ;echo $?)
if [[ ${kel_v} == 0 ]]; then
    printf "Process: Amlogic S9xx\n"
else
    printf "Process: Rockchip RK3328 \n"
fi
...
for sd in $(fdisk -l | grep -E 'Disk /dev/((sd)|(vd)|(hd))' | sed 's/Disk //g' | sed 's/\://g' | awk '{print $1}' | sort); do
    ...
    part_check=$(lsblk "${sd}" | grep -q "part" ;echo $?)
    [[ ${pv_have} -eq 0 ]] && [[ ${part_check} == 1 ]] && C1=$(lsblk ${sd} | sed '1,2d' | awk 'BEGIN {sum = 0} {sum += $4} END {print sum}')
    [[ ${pv_have} -eq 0 ]] && [[ ${part_check} == 0 ]] && C1=$(lsblk ${sd} |  grep "BonusVolGroup-bonusvol" | awk 'BEGIN {sum = 0} {sum += $5} END {print sum}')
    [[ ${pv_have} -eq 0 ]] && C2=$(pvs 2>/dev/null | grep "BonusVolGroup" | grep "${sd}" | awk '{print $6}' | sed -r 's/\.[0-9][0-9]//g' | tr 'a-z' 'A-Z')
    printf "${I1} - "
    if [[ ${pv_have} == 0 ]]; then 
        if [[ ${C1} > 999 ]]; then
            C1=$(${C1} / 1000)
    ...
    if [[ ${part_check} == 1 ]]; then
        ...
        for lvms in $(lsblk ${sd} | awk '{print $2}' | sed -r 's#BonusVolGroup-bonusvol([A-Za-z0-9])#\1#g' | sed 1,2d | sed 's/iqiyi/65543v/' | cut -b 7-20); do
            titles=$(lsblk "${sd}" | grep "${lvms}" | awk '{print $2}' | cut -b 1-6)
            lvm_nam=$(lvs | awk '{print $1}' | grep "${lvms}" | sed -r 's#bonusvol([A-Za-z0-9]+)[0-9]{2}#\1#g' | sed 's/65544v/yunduan/')
            TYPE=${dict[${lvm_nam}]}
            echo -e "${titles}─${TYPE}${lvms:0-2:2} $(df -h | grep "${lvms}" | awk '{print "\t\t", $3, "\t\t", $4, "\t\t", $5}' | sed -r 's#/dev/mapper/BonusVolGroup-bonusvol([A-Za-z0-9])#\1#g' | sed 's/iqiyi/65543v/' | sort)"
        done
